	...
	for {
		...
		if err != nil {
			if errors.Is(err, context.Canceled) {
				logs.Infof("watcher's ctx cancel : %v", err)
				return
			}
			time.Sleep(time.Second)
			logs.Errorf("failed to watch next config: %v", err)
			continue
		}
		if err := c.reader.Merge(kvs...); err != nil {
			logs.Errorf("failed to merge next config: %v", err)
			continue
		}
		if err := c.reader.Resolve(); err != nil {
			logs.Errorf("failed to resolve next config: %v", err)
			continue
		}
...
func (c *config) Load() error {
	for _, src := range c.opts.sources {
		...
		for _, v := range kvs {
			logs.Debugf("config loaded: %s format: %s", v.Key, v.Format)
		}
		if err = c.reader.Merge(kvs...); err != nil {
			logs.Errorf("failed to merge config source: %v", err)
			return err
		}
		w, err := src.Watch()
		if err != nil {
			logs.Errorf("failed to watch config source: %v", err)
			return err
		}
	...
	if err := c.reader.Resolve(); err != nil {
		logs.Errorf("failed to resolve config source: %v", err)
		return err
	}
	return nil
}
